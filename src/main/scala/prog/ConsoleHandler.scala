...
object ConsoleHandler {
  ...
  @tailrec
  def inputHandler(line: String): Array[(Double, Double)] = {
    line.trim.toLowerCase match {
      ...
      case "exit" | "no" | "n" | "учше" =>
        ...
        System.exit(0)
        Array(0.0, 1.0) // костыль чтобы иметь возможность выйти из программы
  ...
  private def chooseRange(func: Double => Double): Array[(Double, Double)] = {
    ...
    for (i <- left to right by (right-left)/12) array = array :+ (i, func(i))
    array
  }
